...
namespace SoundPropagation{
    public class PrimarySoundSource: SoundSourceBase {
        ...
        public void computeTransmission() {
            ...
            for(float angle = 0; angle < 2 * Mathf.PI; angle += gap) {
                ...
                for(int i = offset; i < hits.Length; i++) {
                    ...
                    Obstacle.ObstacleTreeNode depthNode = soundModel.manager.activeGeometry.findObstacleDepth(new Vector2(hit.point) + 0.1f * direction);
        ...
        private void findDiffractionCorners(Obstacle obstacle, bool reversed, Dictionary<Vector2, float> uniqueCorners, List<Obstacle.ObstacleTreeNode> obstacleTree) {
            for(int i = 0; i < obstacle.vertices.Length; i++)
                attemptDiffractionCornerCreation(obstacle, i, reversed, uniqueCorners, obstacleTree);
        }

        public DiffractionSource attemptDiffractionCornerCreation(Obstacle obstacle, int vertexIndex, bool reversed, Dictionary<Vector2, float> uniqueCorners, List<Obstacle.ObstacleTreeNode> obstacleTree) {
